...
Для `approved-plan` после envelope публикуется читаемое содержимое plan внутри `<details>`. Каноническое восстановление и hash verification используют точный payload envelope, поэтому Markdown fences внутри plan не ломают parser.
